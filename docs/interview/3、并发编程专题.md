...
- [并发编程](#并发编程)
  ...
  - [线程池](#线程池)
    ...
    - [线程池之ThreadPoolExecutor详解](#线程池之threadpoolexecutor详解)
      ...
      - [一个简单的线程池Demo: Runnable+ThreadPoolExecutor](#一个简单的线程池demo-runnablethreadpoolexecutor)
...
4. 守护 (Daemon) 线程中不能依靠 finally 块的内容来确保执行关闭或清理资源的逻辑。因为我们上面也说过了一旦所有用户线程都结束运行，守护线程会随 JVM 一。
5. 起结束工作，所以守护 (Daemon) 线程中的 finally 语句块可能无法被执行。
...
![image-20210808220234893](assets/image-20210808220234893.png)
...
能，Java 中可以创建 volatile 类型数组，不过只是一个指向数组的引用，而不是整个数组。意思是，如果改变引用指向的数组，将会受到 volatile 的保护，但是如果多个线程同时改变数组的元素，volatile 标示符就不能起到之前的保护作用了。（只有指向数组的引用会被 volatile 覆盖，对于单个 数组 元素的修改是不会被覆盖的。）
...
1. 所有域都是 final 类型；并且，它被正确创建（创建期间没有发生 this 引用的逸出）。

2. 不可变对象保证了对象的内存可见性，对不可变对象的读取不需要进行额外的同步手段，提升了代码执行效率。
...
![image-20210823203918953](assets/image-20210823203918953.png)
...
![image-20210823204131153](assets/image-20210823204131153.png)
...
![image-20210825164914395](assets/image-20210825164914395.png)
